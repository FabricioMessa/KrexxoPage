document.addEventListener('DOMContentLoaded', () => {
    ...
    const allCharacters = document.querySelectorAll('.personajes .character-card');
    ...
    const allFilterButtons = document.querySelectorAll('.botonLeft, .botonRight, .botonAtributo, .botonChar, .botongenere');

    //Doble filtro 
    const dobleFiltroBtn = document.querySelector('.combinacion1');

    // Correcto selector para los botones de características
    const characteristicButtons = document.querySelectorAll('.caracteristicas .botonChar');
    ...
    function applyFilters() {
        allCharacters.forEach(card => {
            ...
            const matchesGrade = activeFilters.grade.length === 0 || activeFilters.grade.includes(card.dataset.grade);

            const raceValues = card.dataset.race ? card.dataset.race.split(' ').map(r => r.trim()) : [];
            const matchesRace = activeFilters.race.length === 0 || activeFilters.race.some(r => raceValues.includes(r));

            const matchesAttribute = activeFilters.attribute.length === 0 || activeFilters.attribute.includes(card.dataset.attribute);

            const charValues = card.dataset.char ? card.dataset.char.split(' ').map(c => c.trim()) : [];
            const matchesChar = activeFilters.char.length === 0 || activeFilters.char.some(c => charValues.includes(c));

            // 2) AÑADIR LÓGICA PARA GÉNERO
            const genValues = card.dataset.gen ? card.dataset.gen.split(' ').map(g => g.trim()) : [];
            const matchesGen = activeFilters.gen.length === 0 || activeFilters.gen.some(g => genValues.includes(g));

            // === MODO NORMAL (AND ESTRICTO) ===
            if (!isGroupMode) {
                // Todos los "matches" deben ser verdaderos
                if (matchesGrade && matchesRace && matchesAttribute && matchesChar && matchesGen) {
    ...
    function selectRandomTeam(slots) {
        const availableCharacters = Array.from(allCharacters).filter(
            card => !card.classList.contains('banned') && !card.classList.contains('hidden')
        );
        ...
        const selectedCharacters = getRandomCharacters(availableCharacters, slots.length);
    ...
    const nombresRuleta1 = [
        "Sed de sangre", "Campo de batalla ardiente", "Cancelación de ventajas", "Inmunidad a las desventajas",
        "Recuperación de PS", "Muerte inminente", "Infatigable", "Libre de normas", "Sólo cartas de bronce"
    ];
    const nombresRuleta3 = [
        "Sin pasivas", "Mono-verde", "El poder de los pochos", "El poder de los Olvidados", "Mono-demons",
        "mono-humanos", "mono-descos", "mono-azul", "3vs3", "Mono-rojo", "Mirror Match", "Mi waifu favorita", "Llegó el Ragnarok",
        "Esto es una catástrofe", "7 pecados en acción", "Me echas una colaboración?", "Únete a los 10 mandamientos", "Reúnanse Dioses"
    ];
    const colores = [
        "#9900DB", "#000000", "#F00101"
    ];
    //const segmentos = nombres.length;
    //const anguloPorSegmento = (2 * Math.PI) / segmentos;

    // Configurar una ruleta genérica
    function configurarRuleta(canvasId, botonId, resultadoId, nombresPersonalizados = nombres) {
        ...
        function girarRuleta() {
            ...
            function animarGiro() {
                if (velocidad > 0) {
                    ...
                    const indexGanador = Math.floor(anguloFinal / anguloPorSegmentoLocal) % segmentosLocales;
     ...
     */
    function configurarRuletaConEntrada(canvasId, botonId, resultadoId, inputId, agregarBotonId) {
        const canvas = document.getElementById(canvasId);
        canvas.width = 500;
        canvas.height = 500;
        const ctx = canvas.getContext("2d");
        ...
        function girarRuleta() {
            ...
            function animarGiro() {
                if (velocidad > 0) {
                    ...
                    const anguloFinal = (2 * Math.PI - anguloActual) % (2 * Math.PI);
                    const indexGanador =
                        Math.floor(anguloFinal / anguloPorSegmentoLocal) % segmentosLocales;
    ...
    configurarRuleta("canvasRuleta1", "btnRuleta1", "resultadoRuleta1", nombresRuleta1);
    ...
    configurarRuleta("canvasRuleta3", "btnRuleta3", "resultadoRuleta3", nombresRuleta3);
    ...
    selectTeamButton.addEventListener('click', () => {
        ...
        selectedCharactersTeam1 = getRandomCharacters(availableCharacters, teamSlots.length);
    ...
    selectTeamButton2.addEventListener('click', () => {
        ...
        if (availableCharacters.length < teamSlotstoo.length) {
            alert('No hay suficientes personajes disponibles para el segundo equipo.');
            return;
        }

        // Selecciona personajes aleatorios
        const selectedCharactersTeam2 = getRandomCharacters(availableCharacters, teamSlotstoo.length);
    ...
    allCharacters.forEach(card => {
        ...
        desbanButton.addEventListener('click', () => {
            ...
            const anyBanned = [...allCharacters].some(c => c.classList.contains('banned'));
            if (!anyBanned) {
    ...
    const tourmentActionButtons = document.querySelectorAll('.tourment-action-button');
